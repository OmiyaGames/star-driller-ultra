...
public class ShipCamera : UnityStandardAssets.Cameras.PivotBasedCameraRig
{
    ...
    [SerializeField] 
    float m_TurnSpeed = 1; // How fast the rig will turn to keep up with target's rotation
    [SerializeField] 
    float m_RollSpeed = 0.2f;// How fast the rig will roll (around Z axis) to match target's roll.
    [SerializeField] 
    bool m_FollowVelocity = false;// Whether the rig will rotate in the direction of the target's velocity.
    [SerializeField] 
    bool m_FollowTilt = true; // Whether the rig will tilt (around X axis) with the target.
    [SerializeField] 
    float m_SpinTurnLimit = 90;// The threshold beyond which the camera stops following the target's rotation. (used in situations where a car spins out, for example)
    [SerializeField]
    float m_TargetVelocityLowerLimit = 4f;// the minimum velocity above which the camera turns towards the object's velocity. Below this we use the object's forward direction.
    [SerializeField] 
    float m_SmoothTurnTime = 0.2f; // the smoothing for the camera's rotation
